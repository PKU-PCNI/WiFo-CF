...
def main():
    ...
    pretrained_dict = {
        'Standard': './wifo_cf_base.pth',
    }
    # Inference mode
    Result = []
    if args.evaluate:
        for pretrained_name in pretrained_dict.keys():
            print(pretrained_dict[pretrained_name])
            args.pretrained = pretrained_dict[pretrained_name]
            ...
            for scenario in ['Q1.1', 'Q1.2', 'Q1.3', 'Q1.4', 'Q2.1', 'Q2.2', 'Q2.3', 'Q2.4', 'Q3.1', 'Q3.2', 'Q3.3',
                ...
                test_loader = load_data_LH_CDF(args)
                cr_range = [32]
                for cr in cr_range:
                    model.reduction = cr
                    for num_bit in [5]:
                        print(f'Test scenario: {scenario} || Test Cr: {cr} || num bit: {num_bit}\n')
                        loss, rho, nmse, se, se_max = Tester(model, device, criterion, feedback_type='uniform', num_bit=num_bit)(test_loader)
