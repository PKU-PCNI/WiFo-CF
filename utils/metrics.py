...
def Change2complex(h):
    return torch.complex(h[..., 0], h[..., 1]).to(torch.complex128)
...
class SE_Loss(nn.Module):
    ...
    def forward(self, h, h0):
        # input : h:  B, Nt, Nr (complex)      h0: B, Nt, Nr (complex) Ground Truth
        # 1. prepare data
        SNR = self.SNR
        B, Nt, Nr = h.shape
        H = h   # B * Nr * Nt
...
#
# func = SE_Loss(snr=10)
# h = torch.rand((8, 4, 4), dtype=torch.complex64)
# h0 = torch.rand((8, 4, 4), dtype=torch.complex64)
# se, se0 = func(h, h0)
# print(se, se0)
#
# h = torch.rand((8, 4, 4), dtype=torch.complex64)+1
# h0 = torch.rand((8, 4, 4), dtype=torch.complex64)+1
# se, se0 = func(h, h0)
# print(se, se0)
